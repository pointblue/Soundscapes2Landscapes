...
load(file=paste(dpth,"occu_est.RData",sep=""))	#to get optimcovars - the list of VIF selected variables
rastlist<-read.csv(paste(gpth,"covariatesListS2L.csv",sep=""))
rastlist$CovariateFile<-gsub("\\\\","/",rastlist$CovariateFile)
covfiles<-rastlist$CovariateFile
stackvars<-sapply(optimcovars,FUN=function(x,covs){y<-grep(x,covfiles);fv<-covfiles[y];return(fv)},covs=covfiles)
names(stackvars)<-NULL
...
#load the model
load(paste(dpth,"weme_brt.RData",sep=""))
predrast<-predict(covarstack,brt)
