...
pcd<-getSQLforDataFromWH(tablename="ravianpointcountlevel3_v1",spcodes=NA,projectcodes=NA,years=c(2006:2015),extent=ext,con=conn)
asd<-getSQLforDataFromWH(tablename="ravianareasearchlevel3_v1",spcodes=NA,projectcodes=NA,years=c(2006:2015),extent=ext,con=conn)
bnd<-getSQLforDataFromWH(tablename="ravianbandinglevel1_v1",spcodes=NA,projectcodes=NA,years=c(2006:2015),extent=ext,con=conn)	#no data at level 3!
bbd<-getSQLforDataFromWH(tablename="ravianbbsbase_v1",spcodes=NA,projectcodes=NA,years=c(2006:2015),extent=ext,con=conn)
odbcClose(conn)
...
ebd<-getSQLforEBirdData(tablename="ebd_usaonly",commname=NA,states="California",counties=c("Sonoma"),years=c(2006:2015),con=conn)	
odbcClose(conn)

save(pcd,asd,bbd,ebd,file="//prbo.org/Data/Home/Petaluma/lsalas/Documents/lsalas/Mateo/S2Ldata/Sonoma/rawData2006_15.Rdata")

#filter eBird for appropriate protocols and remove dupes
ebdc<-filterEBird(ebd,distlim=0.25, protocols=NA,approv=1,rev=0,allsp=1)
...
effpc<-unique(pcd[,efffields,with=F]);effas<-unique(asd[,efffields,with=F]);
effbb<-unique(bbd[,efffields,with=F])		
...
cali<-readOGR("V:/Data/geopolitical/california/ddWGS84","california")
gdfp<-spTransform(gdf,CRS=projection(cali))
sono<-subset(cali,NAME=="SONOMA")
cnam<-over(gdfp,sono)
...
#Adding this to filter out marsh survey data
effort<-subset(effort,!ProtocolCode %in% c("ISBA_PFLY","VCP10_TM","WATERBIRD_COUNT","SHORE1"))
obsdata<-subset(obsdata,!ProtocolCode %in% c("ISBA_PFLY","VCP10_TM","WATERBIRD_COUNT","SHORE1"))
save(obsdata,effort,file="//prbo.org/Data/Home/Petaluma/lsalas/Documents/lsalas/Mateo/S2Ldata/Sonoma/2006_2015_unmergedData.Rdata")
...
#merged<-merge(effort,obsdata[,c(mergeflds,spflds),with=F],by=mergeflds,all.x=T)