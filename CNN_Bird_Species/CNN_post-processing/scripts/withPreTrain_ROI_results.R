...
libs<-c("ggplot2","plyr","RMySQL","data.table","dplyr")
lapply(libs, require, character.only = TRUE)

## Need the utility matching functions
localPath<-"c:/users/lsalas/git/S2L_devel/"
#localPath<-"c:/users/salasle/CNNpaper/S2Ldevel/"
source(paste0(localPath,"GVanalyses/3models2outputs/scripts/predMatching_utils.R"))

roidata<-read.csv(paste0(localPath,"GVanalyses/3models2outputs/data/pattern_matching_ROIs_201109_testing.csv"), stringsAsFactors=FALSE)
...
allroidata<-read.csv(paste0(localPath,"GVanalyses/3models2outputs/data/pattern_matching_ROIs_centers_210510.csv"), stringsAsFactors=FALSE)
allroidata$event<-sapply(allroidata$filename,function(x){substr(x,1,32)})
...
getROIpredictions<-function(events,speciesmodeled,dbver="s2l_test"){
	...
	for(rr in events){
		...
		preddf<-subset(preddf, PredictionsDatasetId %in% c(2,4,6))
		if(nrow(preddf)>0){
			reslst[[rr]]<-preddf
		}
	}
	...
	if(dbver=="s2l_test"){
		predinfo<-dbGetQuery(con,"select MD.ModelName, PD.PredictionsDatasetId, PD.PredictionScoreType from models as MD inner join predictions_datasets as PD on (MD.ModelId = PD.ModelId) WHERE (PD.PredictionScoreType='sigmoid')")
...
speciesmodeled<-unique(allroidata$birdcode); speciesmodeled<-subset(speciesmodeled,speciesmodeled!="BUSH")
events<-unique(allroidata$event)
labelPreds<-getROIpredictions(events=events,speciesmodeled=speciesmodeled)
labelPreds$ModelName<-ifelse(labelPreds$ModelName=="cols_54cls_Resnet50","Resnet50::sigmoid",ifelse(labelPreds$ModelName=="cols_54cls_Resnet101","Resnet101::sigmoid","MobileNet::sigmoid"))
labelPreds$SamplingEvent<-as.character(labelPreds$SamplingEvent)
labelPreds$SpeciesCode<-as.character(labelPreds$SpeciesCode)
labelPredsList<-list()
labelPredsList[[1]]<-subset(labelPreds,ModelName=="MobileNet::sigmoid")
labelPredsList[[2]]<-subset(labelPreds,ModelName=="Resnet50::sigmoid")
labelPredsList[[3]]<-subset(labelPreds,ModelName=="Resnet101::sigmoid")

save(labelPredsList,file=paste0(localPath,"GVanalyses/data/modelsDataAll/allROI_pretrainedPredictions_210510.RData"))

#########################################

## Load the labelPreds data. It is the entire events where a label was found
load(file=paste0(localPath,"GVanalyses/data/modelsDataAll/allROI_pretrainedPredictions.RData"))

for(nn in 1:3){
	labelPreds<-labelPredsList[[nn]]
	labelPreds<-labelPreds[,c("SamplingEvent","SpeciesCode","Second","PredictionId","PredictionScore","ModelName")]
	## Subset labelPreds to use only the predictions for the test data to make things go faster
	labelPreds<-subset(labelPreds,SamplingEvent %in% unique(roidata$event))
	labelPredsList[[nn]]<-labelPreds
}

roipredsdf<-rbindlist(labelPredsList)

## The models failed to predict in some events with test ROIs... 
pev3<-unique(labelPredsList[[3]]$SamplingEvent)
NROW(pev3)
rev<-unique(roidata$event)
NROW(rev)
rev[which(!rev %in% pev3)]

# Find the matches against the test ROIs
hurdvals<-c(seq(0.65,0.99,0.01),0.999,0.9999,0.99999)
modelNames<-unique(roipredsdf$ModelName)
dataSource="database"
labelSource<-"ROI"

checkNames(predsdf=labelPredsList[[1]],labsdf=roidata,labelSource=labelSource)

tm<-Sys.time()
roimatchlist<-llply(c(modelNames),function(bb,roipredsdf,roidata,hurdvals,dataSource,getROIMatches,removeDoubleROIMatches){
			roipreds<-subset(roipredsdf,ModelName==bb)
			roimatches<-getModelHurdledROIMatches(data=roipreds,roidf=roidata,hurdvals=hurdvals,dataSource=dataSource,getROIMatches=getROIMatches,removeDoubleROIMatches=removeDoubleROIMatches)
			return(roimatches)
		},roipredsdf=roipredsdf,roidata=roidata,hurdvals=hurdvals,dataSource=dataSource,getROIMatches=getROIMatches,removeDoubleROIMatches=removeDoubleROIMatches)
Sys.time()-tm


## check
roimatches<-rbindlist(roimatchlist)
plotmatches<-as.data.frame(subset(roimatches,hurdle==0.65) %>% group_by(SpeciesCode) %>% 
...
ggplot(pdata,aes(x=SpeciesCode,y=Count)) + geom_bar(position="fill",stat="identity",aes(fill=matchType)) + coord_flip() + labs(x="Species",y="Precentage of predictions",title="New method - Pre-trained - Threshold: 0.65")

save(roimatches, file=paste0(localPath,"GVanalyses/3models2outputs/data/withPretrainROIPerformance_06102021.RData"))
