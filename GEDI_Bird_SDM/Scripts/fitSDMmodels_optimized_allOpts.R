...
fitCaseModel<-function(X,logf,ncores=NULL,percent.train=0.8,noise="noised"){
  ...
  ####
  cat("fitSDMmodels Part 1 - Variable definitions and data import \n")
  cat("\n")
  ...
  # Whether to model species or families. This needs to be adjusted manually for now by changing modSpec below
  modSpec = "SPECIES"
  ...
  } else if (modSpec == "FAMILY"){
    # Try with families. Will use same input style, where spcd now equals family name
    fams = c("Emberizidae", "Fringillidae", "Picidae", "Corvidae", "Icteridae", "Paridae")
    Ember = c("CALT", "DEJU", "SOSP", "SPTO", "WCSP")
    Fring = c("AMGO", "HOFI", "LEGO")
    Picid = c("ACWO", "NOFL", "NUWO")
    Corvi = c("STJA", "WESJ")
    Icter = c("BRBL", "RWBL")
    Parid = c("CBCH", "OATI")
    famList=list(Ember, Fring, Picid, Corvi, Icter, Parid)
    names(famList) = fams
    
    keepCols = c(famList[[spcd]], paste0("NumDet", famList[[spcd]]))
    
    spdataPreds = deflatedcovardf[,1:which(colnames(deflatedcovardf)=="ACWO")-1] 
    spdataObs = deflatedcovardf[,which(names(deflatedcovardf) %in% c("x", "y", keepCols))]
    spdataObs[is.na(spdataObs)] = -100
    spdataObs$maxPA = do.call(pmax, spdataObs[,seq(3,ncol(spdataObs),2)])
    aggPA = data.frame(maxPA = spdataObs$maxPA, x = spdataObs$x, y = spdataObs$y)
    colnames(aggPA) = c(spcd, "x", "y")
    aggPA = aggPA %>% dplyr::filter(UQ(as.name(spcd)) >= 0)
    spdataF = dplyr::left_join(spdataPreds, aggPA)
    spdata = as.data.frame(na.omit(spdataF))
    
    predgriddf = spdataPreds[,which(!names(spdataPreds) %in% c("x","y"))]
    predgriddf<-na.omit(predgriddf)
  } else {
    cat("Bad argument for modSpec...\n")
  }
  ...
  ####
  cat("fitSDMmodels Part 2 - Making training and test sets \n")
  cat("\n")
  ...
  } else if (useBal == "spatStratLC"){
    # Adjust presences and absences based on a two-class Land cover map
    cat("Adjusting Presence and Absence spatially based on landcover map \n")
    
    # Extract land cover value for stratification
    LCras = raster("G:/NAU/Projects/GEDI/SubProj/Sonoma_biodiv/MOD_LC/modLC1_2013to15_remap_463m-EPSG32610.tif") # need to change this for monsoon
    xycols = c("x", "y")
    xydf_LC = xydf
    xypts_LC = SpatialPointsDataFrame(data = xydf_LC, coords = xydf_LC[,xycols])
    crs(xypts_LC) = crs(LCras)
    xydf_LC$LC = raster::extract(x = LCras, y = xypts_LC)
    spdata_LC = dplyr::left_join(x = spdata, y = xydf_LC, by = c("x", "y"))
    spdata_LC$PA = spdata_LC$PresAbs
    spdata_LCpts = SpatialPointsDataFrame(data = spdata_LC, coords = spdata_LC[,xycols])
    crs(spdata_LCpts) = crs(LCras)
    
    # Count the number of "natural" presences and then get equal number of "natural" absences
    natPres = spdata_LC[spdata_LC$PA == 1 & spdata_LC$LC == 1,]
    numNatPres = nrow(natPres)
    
    natAbs = spdata_LC %>% dplyr::filter(PA == 0 & LC == 1) 
    natAbsSamp = natAbs %>% dplyr::sample_n(numNatPres, replace = F)
    
    # Now get "unnatural" P and A based on proportion of this area
    unnatPres = spdata_LC %>% dplyr::filter(PA == 1 & LC == 0) 
    
    if (nrow(unnatPres) < ceiling(numNatPres/4)){
      sampSizePres = nrow(unnatPres)
    } else {
      sampSizePres = ceiling(numNatPres/4)
    }
    unnatPresSamp = unnatPres %>% dplyr::sample_n(sampSizePres, replace = F)
    
    
    unnatAbs = spdata_LC %>% dplyr::filter(PA == 0 & LC == 0) 
    
    if (nrow(unnatAbs) < ceiling(numNatPres/4)){
      sampSizeAbs = nrow(unnatPres)
    } else {
      sampSizeAbs = ceiling(numNatPres/4)
    }
    unnatAbsSamp = unnatAbs %>% dplyr::sample_n(sampSizeAbs, replace = F)
    
    # Combine all subsets into one df
    PA_strat = rbind(natPres, natAbsSamp, unnatPresSamp, unnatAbsSamp)
    
    # Split into train and test
    H<-holdout(PA_strat$PresAbs_f,ratio=percent.train)
    trainset<-PA_strat[H$tr,];testset<-PA_strat[H$ts,]
    
    trainset<-trainset[,which(!names(trainset) %in% c("LC","PA","cellId"))]
    testset<-testset[,which(!names(testset) %in% c("LC","PA","cellId"))]
  ...
  #### 3. Fit Models
  ####
  cat("fitSDMmodels Part 3 - Fitting models \n")
  cat("\n")
  ...
  # Additional models which have issues and are currently not included
  # print("Fitting MARS")
  # grid search AUC values don't look right...
  # marssearch=list(smethod="grid", search=list(degree=c(1,2,3,4), nk=c(2,4,8,16,20)), convex=0, metric="AUC", method=c("kfold",5))
  # mars<-try(fit(x = as.formula(fmln), data = trainset, model = "mars",
  #               eval_metric = "auc", search = marssearch, scale = "none", prune=TRUE,
  #               task = "prob", transform = "none", fdebug=TRUE), silent=TRUE)
  # # # print(" ")
  
  # print("Fitting XGBoost")
  # errors out while fitting...
  # xgbsearch=list(smethod="grid", search=list(gamma=c(1,2,3), max_depth=c(2,3,4,5,6), 
  #                                            min_child_weight=c(0.5,1.0,1.5), colsample_bytree=c(0.4,0.7,1.0)), 
  #                convex=0, metric="AUC", method=c("kfold",5))
  # xgb_RM<-try(fit(x = as.formula(fmlf), data = trainset, model = "xgboost", objective="binary:logistic",
  #               search = xgbsearch, scale = "none", nrounds = 100, eta=1, eval_metric="auc",
  #               task = "prob", transform = "none", fdebug=TRUE), silent=TRUE)
  
  # Models to run (these work)
  # Run with rminer
  ...
  if((inherits(rfom,"try-error") || class(rfom@object)=="character") && (inherits(svmm,"try-error") || class(svmm@object)=="character") && 
    ...
    ####
    cat("fitSDMmodels Part 4 - Calculating variable importance \n")
    cat("\n")
    # Use rminer methods
    ...
    #### 5. Model predictions
    ####
    cat("fitSDMmodels Part 5 - Model predictions \n")
    cat("\n")
    ...
    ####
    cat("fitSDMmodels Part 6 - Goodness of Fit (GOF) \n")
    cat("\n")
    ...
    #### 7. Model ensemble
    ####
    cat("fitSDMmodels Part 7 - Model ensemble for this iteration \n")
    cat("\n")
    ...
    # Remove "bad" models before weighted average
    # Filters: (1) AUC > 0.5, (2) truePos > 0 & trueNeg > 0, (3) number of features > 1
    ...
    if(nrow(gofs_filt)>=2){
      ...
      allD_WA = alldataLT_filt %>% select(c("observed", "weighted"))
      trte_WA = trte_res_LT_filt %>% select(c("observed", "weighted"))
    ...
    #### 8. Export maps
    ####
    cat("fitSDMmodels Part 8 - Saving maps \n")
    cat("\n")
    ...
    # Save training, test, and models as one .RData file. Takes up a lot of space. Commented out for now
